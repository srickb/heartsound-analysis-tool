...
| --- | --- | --- | --- | --- | --- |
| 사용목적(Intended Use) 확장 리스크 | 연구용 Tool 문맥에서 질환 판단, 이상 평가, 스크리닝, 위험도 제시 등 의료 목적 표현 사용 | 연구용 Tool → 디지털의료기기/독립형 디지털의료기기소프트웨어로 규제 포지셔닝 상향 가능성 | 연구용 신호 분석·해석 지원 Tool로 intended use 고정, 문서 표현 일치 | intended use 변경 이력 관리, 대외 문서 검토 프로세스 | Product Owner / Regulatory Review Owner |
| 문서 간 표현 불일치 리스크 | PRD, 발표자료, README, UI, export, 논문/포스터 간 기능 표현 불일치 | 연구용 Tool 주장 약화, 임상 판단 보조/자동 판정 기능으로 오해 가능성 | 핵심 용어 사전 운영, 허용 표현 통일 | 표현 검토 체크리스트, 승인 절차 수립 | Product Owner / Documentation Owner |
| 진단적 표현 사용 리스크 | `진단`, `확진`, `자동 판정`, `위험도`, `정상/비정상`, `의심 소견`, `스크리닝 결과` 등 사용 | 질병 판단 목적과 직접 연결, 규제 부담 상승 | 진단적 표현 배제, 이벤트 검출/파라미터 산출/시각화/탐색적 review 중심 표현 유지 | 금지 표현 목록, 허용 표현 목록 관리 | Product Owner / UI Owner |
| S3/S4 오해 유발 리스크 | S3/S4 candidate를 확정 이벤트 또는 병적 의미로 제시 | 임상적 결론으로 오해, 제품 성격 보수적 재해석 가능성 | candidate overlay로만 표기, confirmed structure와 시각적 분리 | tooltip/legend/export 정책 보강 | Algorithm Owner / UI Owner / Clinical Review Owner |
| 파라미터의 임상 해석 직접 연결 리스크 | parameter를 환자 상태, 질환 가능성, 이상 여부와 직접 연결 | 단순 분석 기능 → 의료 판단 보조 기능으로 해석 가능성 | cycle-centered review metric으로만 제시, 임상 결론과 직접 분리 | parameter별 intended meaning, 허용/금지 해석 범위 문서화 | Algorithm Owner / Clinical Review Owner |
| 자동 판정 기능으로의 기능 확장 리스크 | 이상 여부, 질환 가능성, 경고 flag, 선별 결과 자동 생성 | 규제 대상성 확대, 검증·성능 입증·사용적합성 요구 증가 | 자동 판정 기능 비목표 유지 | Stage 2 intended use 재확정, 허가 전략 검토 | Product Owner / Regulatory Review Owner |
| 의료진 판단 영향 리스크 | 제품 출력이 진료 판단, 추가 검사, 추적관찰 결정에 실질적 영향 | 해석 지원 범위 초과, 임상 판단 보조 SW로 분류 검토 가능성 | 내부 연구 검토용 사용 범위 유지 | 사용 시나리오, 사용자, 사용 환경 명확화 | Product Owner / Clinical Review Owner |
| 배포 범위 확대 리스크 | 연구실 내부 Tool의 외부 기관/병원/다수 사용자 반복 배포 | 제품 수준 접근통제, 로그, 업데이트, 보안 요구 증가 | local-first, 제한적 temporary share 유지 | 계정 체계, 권한 관리, 업데이트/배포 정책 수립 | Engineering Owner / Security Owner |
| Export 결과의 오해 리스크 | sheet명, column명, flag명, 주석이 임상 판단 결과처럼 보이는 경우 | export 결과의 임상 판단 자료화, 규제 리스크 확대 | cycle anchor 기반 parameter summary 유지, candidate 결과 분리 | export naming convention, 메타데이터 경고문, 버전 정보 정책 수립 | Data Owner / Product Owner |
| AI 기능 설명 부족 리스크 | AI 추출, rule-based 후처리, candidate detection, parameter 산출 구분 불명확 | 불투명한 자동 판단 시스템으로 인식, 검증·심사 대응 어려움 | AI / rule-based / 시각화 / export 로직 분리 문서화 | 입력/출력/실패 조건/검증 방법 traceability 관리 | Algorithm Owner / Validation Owner |
| 검증 부족 상태의 과대 주장 리스크 | pilot 수준 성능으로 정확도, 신뢰도, 임상 활용 가능성 과대 홍보 | 성능 claim 과장, 규제·신뢰성 리스크 동시 발생 | 내부 개발 게이트 기준만 사용, 임상 효능 표현 배제 | claim 관리 문서, 발표 승인 절차, 데이터셋 범위 명시 | Validation Owner / Product Owner |
| 사용자 오사용 리스크 | candidate overlay, parameter card, graph annotation을 확정 판정으로 해석 | 사용적합성 리스크, 규제 오해 리스크 동시 발생 | candidate/confirmed structure 구분, 설명 문구 명확화 | 사용 오류 시나리오 정의, 사용적합성 평가 계획 수립 | UI Owner / Usability Owner |
| 보안 및 접근통제 미흡 리스크 | temporary public share, lightweight gating, code mode의 외부 확장 사용 | 접근통제, audit, 보안 문서 요구 대응 한계 | 연구용 편의 기능으로 한정, 제품 기본 구조로 간주하지 않음 | 계정, 권한, 접근 로그, 보안 정책 재설계 | Security Owner / Engineering Owner |
| 변경관리 미흡 리스크 | 알고리즘, UI, formula, export 변경 대비 versioning/change log 부족 | 검증 결과와 배포 버전 간 연결성 약화 | spec, formula reference, PRD, validation record 분리 관리 | traceability table, release note, defect log, 재검증 기준 도입 | QA Owner / Validation Owner |
| 규제 전략 지연 리스크 | 기능 확장 대비 intended use, 사용자, 사용 환경, 허가 전략 검토 지연 | 개발 방향-규제 대응 방향 불일치, 재작업 비용 증가 | Stage 1 연구용 범위 명확화 | Stage 2 이전 규제 포지셔닝 사전 검토, 책임자 지정 | Product Owner / Regulatory Review Owner |
...
본 문서는 HeartSound(PCG) 및 ECG 기반 신호 분석 Tool의 현재 구현 범위, 제품 목표, 기능 요구사항, 비기능 요구사항, 데이터 및 알고리즘 요구사항, 검증 방향, 그리고 향후 허가·규제 대응 방향을 일관된 구조로 정리하기 위함.
...
본 제품은 HeartSound(PCG) 및 ECG Signal을 업로드하고, panel 단위로 파일을 연결하여, waveform 시각화, cycle 기반 이벤트 검토, 파라미터 계산, audio playback, 비교 분석, xlsx export를 수행할 수 있는 연구용 신호 분석 소프트웨어이다.
**ECG Signal 보완 필요**
...
### PCG 데이터
- **데이터 출처**: PhysioNet
- **데이터셋명**: `The CirCor DigiScope Phonocardiogram Dataset`
- **데이터 성격**: 공개 데이터셋
